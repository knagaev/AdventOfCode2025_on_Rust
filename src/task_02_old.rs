...
fn calc_invalid_ids_sum(ids_range: &str) -> Option<u128> {
    ...
    println!("start_range {:?} end_range {:?}", start_range, end_range);
...
fn calc_atleast_invalid_ids_sum(ids_range: &str) -> Option<u128> {
    let (start_range, end_range) = ids_range.split_once('-')?;
    ...
    for i in 1..=start_width / 2 {
        for d in 10.pow(i - 1)..=10.pow(i) - 1 {
            let mut id = 0_u128;
            for j in 0..start_width / i {
                id += d.po
            }
        }
    }
